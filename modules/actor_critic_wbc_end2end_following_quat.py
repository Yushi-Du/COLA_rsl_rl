...
class ActorCriticWbcEnd2endFollowingWholePipeQuat(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        critic_for_predicted_commands_layers.append(nn.Linear(self.critic_for_predicted_commands_obs_dim, critic_hidden_dims[0]))
    ...
    def actor_forward(self, observations, inference=False):
        ...
        if self.env._actor_cnn_step < self.env.stage_two_steps:
            if not inference:
                ...
                print(f"predicted:{predicted_command[0]}, original:{original_commands[:, -1, :][0]}")

            final_commands = original_commands  # command_predictor
        else:
            final_commands = predicted_commands_history

        # final_commands = original_commands
        try:
