...
class StudentTeacherDistill_Resi_HM(nn.Module):
    is_recurrent = False
    ...
    def _build_residual_teacher(self, num_teacher_obs, num_actions, teacher_hidden_dims, activation):
        ...
        self.teacher_base_obs_dim = num_teacher_obs - self.history_length * (13) - 187
...
class TeacherResidualWrapper(nn.Module):
    ...
    def forward(self, observations):
        observations_no_hm = observations[:, :-187]
        flattened_obs = observations_no_hm.reshape(observations_no_hm.shape[0], self.history_length, -1)
        ...
        # The residual MLP is unbounded and explodes on OOD states (student falls,
        # impact joint-vel spikes, its own large past actions in the obs -- a
        # self-amplifying loop measured up to |a|~1e5, poisoning the BC targets;
        # nominal |a|max is ~7). Clamp targets so garbage states stay bounded.
