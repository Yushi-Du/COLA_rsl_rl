...
class StudentTeacherDistill_Resi(nn.Module):
    is_recurrent = False
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # student_locomotion
        student_locomotion_layers = []
        student_locomotion_layers.append(nn.Linear(mlp_input_dim_s, student_hidden_dims[0]))
        student_locomotion_layers.append(activation)
        for layer_index in range(len(student_hidden_dims)):
            if layer_index == len(student_hidden_dims) - 1:
                student_locomotion_layers.append(nn.Linear(student_hidden_dims[layer_index], num_actions))
            else:
                student_locomotion_layers.append(nn.Linear(student_hidden_dims[layer_index], student_hidden_dims[layer_index + 1]))
                student_locomotion_layers.append(activation)
        self.student_locomotion = nn.Sequential(*student_locomotion_layers)
    ...
    def update_distribution(self, observations):
        mean = self.student_locomotion(observations)+self.student(observations)
    ...
    def act_inference(self, observations):
        actions_mean = self.student_locomotion(observations)+self.student(observations)
        return actions_mean
    ...
    def load_state_dict(self, state_dict, strict=True):
        ...
        if any("residual" in key for key in state_dict.keys()):  # loading parameters from rl training
            # rename keys to match teacher and remove critic parameters
            teacher_state_dict = {}
            student_state_dict = {}
            student_locomotion_state_dict = {}
            ...
            for key, value in state_dict.items():
                ...
                if "actor." in key and "critic" not in key:
                    if "residual" in key:
                        ...
                        student_locomotion_state_dict[key.replace("actor.", "")] = value
                if "student" in key:
                    student_state_dict[key.replace("student.", "")] = value
            print(teacher_state_dict.keys())
            print(student_state_dict.keys())
            print(student_locomotion_state_dict.keys())
            self.teacher.load_state_dict(teacher_state_dict, strict=strict)
            self.student_locomotion.load_state_dict(student_locomotion_state_dict, strict=strict)
